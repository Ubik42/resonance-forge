...
![Version](https://img.shields.io/badge/Version-0.32.0-D96B2B)
...
- **可演奏**：面板可发现并连接 MIDI 输入设备；Note On 控制音高与力度，原生弓擦按住持续、Note Off 进入 140 ms 收弓。持音中推动 CC1 会平滑改变弓速与弓压，并同步更新 Wwise `RF_ImpactBrightness`。
...
13. 先用“试音键床”点击同一位置，再依次切换软触、线性、重手三枚“力度凸轮”；观察输入力度不变而输出能量和曲线落点改变。有 MIDI 键盘时，在“演奏入口”连接设备；选择弓擦并按住一个音符，缓慢推动调制轮，听弓压与亮度在同一持续音中变化。
...
| CC1 调制轮 | 原生持音弓擦的实时弓速/弓压，并同步到 Wwise `RF_ImpactBrightness` |
...
| MIDI 持弓与 CC1 表现 | Note On 持弓 3 秒后仍保留持续能量，Note Off 后进入 140 ms 收弓；同一持音从 CC1 `18%` 推至 `92%` 时，稳定段 RMS 从 `0.024450` 变为 `0.024705`，参数变化无需重新触发音符 |
...
- 面板按钮与鼠标键床仍使用有限自动弓程；硬件 MIDI 已支持 Note On 持弓、CC1 连续表现与 Note Off 收弓。当前 CC1 同时耦合弓速、弓压和明亮度，尚未拆成独立控制，也未提供换弓或双向弦段耦合。
