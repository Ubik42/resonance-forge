...
![Version](https://img.shields.io/badge/Version-0.23.0-D96B2B)
...
第二张呈现数字波导弦的“弦床”、材质听感、三档锤击标尺、Wwise 出口刻度、本地配方槽，以及能判断尾音、切换最近版本的“铸样台 / 余响拓片 / 铭牌架”。
...
数字弦的“回响长度”是面向设计师的归一化控制，内部映射到 `0.9700–0.9995` 的反馈系数；接近上限时衰减会显著变长。“弦路阻尼”决定环路低通造成的高频耗散，“箱体耦合”决定弦能量进入模态共振体的比例。“拾音位置”把琴桥到弦心映射到延迟线长度的 `4%–50%`，将当前波与该位置的延迟波做双点差分，因此改变的是实际输出频谱，而不是只改界面曲线。它是轻量数字波导的演奏性控制，不声称复刻琴体、弦列与电磁拾音器的完整有限元模型。
...
12. 切换到“数字波导弦”，调节弦床中的回响长度、弦路阻尼、箱体耦合与“琴桥 ↔ 弦心”拾音位置，观察声纹与听感同步变化。先固定其余三项，只移动拾音位置，最容易听清谐波缺口的变化。
