...
![Version](https://img.shields.io/badge/Version-0.26.0-D96B2B)
...
没有 MIDI 硬件时也可以演奏。试音键床提供 C3–C4 的十三根锤键，横向决定音高，纵向决定力度；点击或拖奏会走和外接 MIDI 相同的 UE/Wwise 触发链。键床下方先用“起振手势架”选择指腹、拨片或锤击，再在“弦路尺”上把青色拾音梭拖到琴桥与弦心之间。
...
| 数字波导弦 | 指腹/拨片/锤击初始条件、延迟线传播、环路低通、衰减反馈、双点拾音 | 弦、金属丝、可演奏机关 | 起振手势、MIDI 音高、力度、阻尼、反馈、材质耦合与拾音位置 |
...
“起振手势”只改变延迟线的初始条件：指腹使用低通相关噪声乘半弦空间包络，拨片使用亮度控制的短噪声位移，锤击在当前落点形成局部双极脉冲。音高、反馈、阻尼、箱体与拾音仍走同一后续弦路，因此可以固定其余参数做干净比较。它是轻量数字波导的演奏性控制，不声称复刻琴槌毡层、弦间耦合、音板辐射等 Pianoteq 级完整钢琴模型。
...
13. 切换到“数字波导弦”，先在“起振手势架”依次试听指腹、拨片与锤击；再调节回响长度、弦路阻尼和箱体耦合，并在弦路尺上把青色拾音梭从琴桥拖向弦心。每次比较只改变一项，最容易听清攻击与谐波缺口的差别。
...
```json
{
  ...
  "audio": { "file": "RF_FingerString_G3.wav", "sampleRate": 48000, "tailRelativeDb": -32.0 },
  "source": { "preset": "硬木", "model": "WaveguideString", "midiNote": 55, "velocity": 0.76 },
  "waveguide": { "feedback": 0.99124, "damping": 0.52, "bodyCoupling": 0.26, "pickupPosition": 0.82, "excitation": "Finger" },
...
| 起振手势隔离比较 | 固定 Note、力度、明亮度、回响、阻尼、耦合与拾音位置，仅将指腹切为锤击；两份 0.26 WAV 的差分 RMS 为 `0.00598`、相关系数为 `0.94163`，并非换名复制 |
...
| 铭牌架往返 | 在其余参数相同时自动铸出指腹与锤击两版，再按指腹铭牌确切路径恢复硬木波导弦、起振手势、6 模态、Note 55、76/58/46% 参数与 180 段拓片；铭牌架同步标记当前回炉版 |
| Wwise 生成资源检查 | 通过 |
