...
![Version](https://img.shields.io/badge/Version-0.43.0-D96B2B)
...
从场景里选中一个物体，先用 PIE 碰撞或 MIDI / 鼠标演奏让它起振，再调整模态、弦路与监听出口。满意的声音可以保存成 UE 配方资产，也可以铸成带参数铭牌的 WAV，继续交给 Wwise 或 DAW。
...
以下图片由插件自身的 Slate 截图链路直接导出，不依赖界面拼接。顶部“冲量标定砧尺”把静默门槛、半响、满响与对象采样画在同一条标尺上；它先回答“为什么这次碰撞没有响”，再把通过门槛的冲量交给后续声学工位。

铸样区保留最近三份可回炉的声源铭牌，并用真实振幅包络提示尾音是否被截断。每张铭牌都能单独收入或移出 REAPER 对照带，工作台会把真正要比较的版本画在同一条时间轨上。

![共振铸造台弦床与配方架](docs/images/resonance-forge-workbench-details.png)

Wwise 路由区把钢、木、玻璃三条 Event 线与能量、明亮度、尺度三条参数线直接铺开，并标明当前来源；完整绑定可铸成独立 Content 路由资产，在不同地图和项目中复用。

![共振铸造台 Wwise 路由织机](docs/images/resonance-forge-wwise-route.png)

模态撞击体不是固定预设。落点划线规选择物体上的敲击位置，短齿显示各模态的受激程度；“共振齿列”把每一个模态放在对数频率轴上，允许逐根调整并试听。

![共振铸造台可编辑共振齿列](docs/images/resonance-forge-mode-rack.png)

没有 MIDI 硬件时也可以演奏。“弓行轨”把鼠标动作变成推弓、回弓、弓速与弓压；试音键床提供 C3–C4 的十三根锤键，“力度凸轮”再用软触、线性或重手曲线处理输入力度。顶部配方对照尺只标出 A / B 两版真正发生变化的声学工位。

![共振铸造台试音键床](docs/images/resonance-forge-keybed.png)

演示地图提供钢、木、玻璃落球砧座与数字波导弦，用于观察材质、碰撞量级和声音出口之间的关系；场景美术只承担测试语境，不是插件运行依赖。
...
- **碰撞拓印与自动回标**：每个对象的砧尺保留最近 12 次 PIE 碰撞，区分未过门槛与成功起声；采满至少三次后可按弱端/强端分位点重算门槛和灵敏度，并按 Actor 标签把结果从 PIE 临时副本带回编辑地图。切换碰撞对象时自动换册，不混算不同物件。
...
- **可拖动的弓感双轮**：左轮直接调弓速/明亮度并同步 Wwise Brightness，右轮直接调弓压；首次拖动会切到数字波导弦的弓擦手势，拖动时更新当前持音，松手用有限自动弓程试听。连接 MIDI 后，两轮继续显示 CC1、Aftertouch 与回退状态。
- **可以直接拉奏的弓行轨**：按住弓座建立原生持音，横向速度进入摩擦速度，移动方向在 `+1/-1` 间平滑换弓，纵向位置进入弓压；松手发送 Note Off 并进入 140 ms 收弓。MIDI 正在持音时会阻止轨道起弓，避免误收同音高的外部音符。
...
- **REAPER 对照带**：从铭牌架勾选 1–3 份有效铸样，按“更早 → 当前”排入同一条 48 kHz 轨道；选择变化后旧排带会失效，重新生成的 `.rpp` 使用同目录相对引用，整个 Exports 文件夹可以一起移动。
...
- **三格铭牌架**：最近三份铸样按铭牌 UTC 时间自动上架，直接显示文件名、材质、模型、Note、时长与尾音读数；点击铭牌可以回炉，下方拨片决定该版本是否进入下一条 REAPER 对照带。
...
- **配方级 A/B 对照**：钉住本炉配方后，声材、模型、起振手势、Note、输入力度、力度曲线、弓速、弓压、弓向、弦床与共振齿会一起进入参考快照。“配方对照尺”用两条连续工艺轨和差异立柱解释改了什么；交换试听会恢复完整演奏状态，不只交换声纹轮廓。
- **本地配方铭牌架**：甲、乙、丙三个槽位保存材质、模型、完整共振齿列、落点/起振点、波导弦参数、手势、Note、输入力度、弓速、弓压与弓向。铭牌直接显示关键差异，重开编辑器后可一键回炉并试听；旧版槽位缺少的字段会使用兼容默认值。
...
指腹、拨片和锤击改变延迟线的初始条件：指腹使用围绕起振点的宽缓平滑位移，拨片形成以起振点为峰值的三角位移并叠加亮度可塑噪声，锤击在起振点形成局部双极脉冲。弓擦不同：弓点处每个采样都计算“带方向的弓速－弦速”，经过形如 STK `BowTable` 的非线性摩擦曲线后持续补能，并用 25 ms 起弓与 140 ms 收弓包络避免硬切。换弓时方向目标在 10 ms 内穿过零点，避免把正负速度硬切成爆点。鼠标键床与双轮试听保持约 `0.85–3.2` 秒的自动弓程；弓行轨和硬件 MIDI 使用持音语音，松开轨道或 Note Off 才进入收弓。持音时 CC1 经过 18 ms 平滑后改变弓速，Aftertouch 经过 12 ms 平滑后独立改变弓压；未收到 Aftertouch 前，弓压跟随 CC1。
...
10. 点击“钉住本炉配方”，再切换材质、模态齿、手势或演奏参数；配方对照尺会把 A 参考与 B 本炉的差异沿六个工位标出。
11. 点击“交换并试听 A/B”在两版之间往返切换；材质、模型、模态数组、弦床、Note、力度曲线和弓感状态都会随版本交换。
...
15. 在“铸样台”输入文件名并选择 1.5、3 或 6 秒，点击“铸成 WAV”；观察“余响拓片”是否提示尾音仍活跃，必要时延长一档或降低回响长度。工具会成对写入 WAV 与 `.rfrecipe.json` 声源铭牌，重名时自动追加编号。下方“铭牌架”会刷新最近三版：点击铭牌可回炉，用每格下方的拨片收入或移出对照带，最后点击“排成 .rpp”。
...
```json
{
  ...
  "waveguide": { "feedback": 0.99124, "damping": 0.52, "bodyCoupling": 0.26, "pickupPosition": 0.82, "excitation": "Bow", "bowSpeed": 0.55, "bowPressure": 0.68, "bowDirection": "Reverse" },
...
“REAPER 对照带”只处理铭牌架中明确收入的 1–3 个版本，并复检对应 WAV 的 RIFF 头、48 kHz / 16-bit / stereo 规格和真实时长。有效版本按“更早 → 当前”写入 `ResonanceForge_Audition.rpp` 的同一轨道，每段之间留 0.5 秒，Item 名包含文件、材质与声源模型。移出或收入任一版本后，工作台会禁用旧工程入口，直到重新排带，避免把旧选择误当成当前结果。RPP 与 WAV 位于同一目录并使用相对文件名；它不复制音频，也不预设插件、混音、Render Queue 或母带链。未安装 REAPER 时仍可先生成工程，只有点击“打开 REAPER 工程”才请求系统启动关联程序。

### 从个人草案到团队资产

“配方铭牌架”与 Content 资产承担两种不同用途：

- **甲、乙、丙槽**：保存在本机 `EditorPerProjectUserSettings`，适合临时试音和个人 A/B，不进入版本控制。每格铭牌会显示模型、手势、Note、弓速/弓压或撞击落点，以及共振齿数量；“回炉试听”会原子地恢复整套参数后触发一次当前声音。
...
| 碰撞拓印与回标 | 同一对象最近 12 次冲量进入工作台采样缓存；至少 3 次且跨度充分时按 10% / 90% 分位端点回标，切换对象会换册，PIE 内操作同步写回同标签的编辑世界 Actor |
| 内置声学预设测试 | 通过 |
...
| REAPER 对照带 | 自动截图从最近三份铭牌中移出中间版本，其余两份按 `0.0 / 3.5` 秒排入单轨 RPP；选择状态、同目录相对 WAV 引用和 REAPER 7.78 x64 工程接收均已验证 |
...
| 弓感双轮松手试听 | 同音高、同能量、同弓速下，仅把自动弓程压力从 `18%` 改为 `92%`，两份输出差分 RMS 为 `0.012395`；证明右轮参数进入实际 Strike 快照，而非只更新表针 |
| 弓行轨换弓 | 固定 Note 55、能量 82%、弓速 58%、弓压 68% 和全部弦床参数，只把波导摩擦速度方向从推弓 `+1` 改为回弓 `−1`；两份一秒输出差分 RMS 为 `0.047367` |
| 本地配方铭牌往返 | 自动截图路径把硬木波导弦、弓擦、G3、回弓、`55%` 弓速、`68%` 弓压与 6 根共振齿压入甲槽，再把弓压改为 `24%` 后回炉；实机铭牌恢复并显示“回弓 / `68%`”，随后以 Note 55 试听 |
...
- 面板按钮、鼠标键床与双轮微调仍使用有限自动弓程；弓行轨支持推弓/回弓、实时弓速/弓压和松手收弓，硬件 MIDI 支持 Note On 持弓、CC1 弓速、Aftertouch 弓压与 Note Off 收弓。尚未建模弓毛接触位置、松香状态或双向弦段耦合。
...
- 铭牌架当前只允许在本工程铸样目录最近三份 v1 文件之间选择；尚未提供任意路径、搜索或批量导入，也不会自动修改 Wwise 工程。
