...
![Version](https://img.shields.io/badge/Version-0.24.0-D96B2B)
...
没有 MIDI 硬件时也可以演奏。试音键床提供 C3–C4 的十三根锤键，横向决定音高，纵向决定力度；点击或拖奏会走和外接 MIDI 相同的 UE/Wwise 触发链。键床下方的“弦路尺”把拾音位置放回一根可见的弦上，可直接在琴桥与弦心之间拖动青色拾音梭。
...
- **可塑形的数字弦**：“弦床”直接控制回响长度、弦路阻尼、箱体耦合和拾音位置；琴桥到弦心的两点取样差会形成可听的梳状频谱变化。声音、声纹、A/B 参考、配方槽与声源铭牌使用同一组参数。
- **可拖动的弦路尺**：不把拾音位置继续做成第四根抽象滑杆；铜色驻波包络、琴桥基座、弦心刻线和青色拾音梭共同解释声音为何变化，拖动时塑形、松手时试听。
...
| 数字波导弦 | 噪声激励、延迟线传播、环路低通、衰减反馈、双点拾音 | 弦、金属丝、可演奏机关 | MIDI 音高、力度、阻尼、反馈、材质耦合与拾音位置 |

数字弦的“回响长度”是面向设计师的归一化控制，内部映射到 `0.9700–0.9995` 的反馈系数；接近上限时衰减会显著变长。“弦路阻尼”决定环路低通造成的高频耗散，“箱体耦合”决定弦能量进入模态共振体的比例。“拾音位置”把琴桥到弦心映射到延迟线长度的 `4%–50%`，将当前波与该位置的延迟波做双点差分，因此改变的是实际输出频谱，而不是只改界面曲线。弦路尺中的驻波外形受回响与阻尼驱动，琴桥下的共鸣基座受耦合驱动，青色拾音梭则直接写入同一个 DSP 参数。它是轻量数字波导的演奏性控制，不声称复刻琴体、弦列与电磁拾音器的完整有限元模型。
...
7. 再细调激励能量、明亮度与共振尺度：拖动时观察声纹和出口刻度，松手试听一次。
...
12. 切换到“数字波导弦”，调节弦床中的回响长度、弦路阻尼和箱体耦合，再在弦路尺上把青色拾音梭从琴桥拖向弦心。先固定前三项，只移动拾音梭，最容易听清谐波缺口的变化；拖动过程只塑形，松手试听一次。
13. 在“铸样台”输入文件名并选择 1.5、3 或 6 秒，点击“铸成 WAV”；观察“余响拓片”是否提示尾音仍活跃，必要时延长一档或降低回响长度。工具会成对写入 WAV 与 `.rfrecipe.json` 声源铭牌，重名时自动追加编号。下方“铭牌架”会刷新最近三版，点击任意铭牌即可回炉；“回炉最近铭牌”仍保留为最快入口。
...
```json
{
  ...
  "audio": { "file": "RF_StringCenter_G3.wav", "sampleRate": 48000, "tailRelativeDb": -35.7 },
  "source": { "preset": "硬木", "model": "WaveguideString", "midiNote": 55, "velocity": 0.76 },
  "waveguide": { "feedback": 0.99124, "damping": 0.52, "bodyCoupling": 0.26, "pickupPosition": 0.82 },
...
“长尾木弦”使用硬木六模态、`0.9987` 反馈、`0.22` 阻尼、`0.44` 箱体耦合和 `0.82` 弦心侧拾音位置。基础地图与可选 Fab 地图中的数字波导弦默认引用该资产；磁盘重载复检会同时检查资产路径和四项参数。
...
| 铭牌架往返 | 自动铸出长尾木弦与干燥木弦两版，再按长尾铭牌确切路径恢复硬木波导弦、6 模态、Note 55、76/58/46% 参数与 180 段拓片；铭牌架同步标记当前回炉版 |
| Wwise 生成资源检查 | 通过 |
