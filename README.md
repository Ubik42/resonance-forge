...
![Version](https://img.shields.io/badge/Version-0.36.0-D96B2B)
...
没有 MIDI 硬件时也可以演奏。演奏入口先用“弓感双轮”把弓速、弓压及其回退关系画成可读的机械标尺；直接拖左轮或右轮，松手就会用当前自动弓程试听一次。试音键床提供 C3–C4 的十三根锤键，横向决定音高，纵向决定原始力度；“力度凸轮”再用软触、线性或重手曲线把输入映射成实际激励能量。点击或拖奏会走和外接 MIDI 相同的 UE/Wwise 触发链。键床下方可以继续选择起振手势，并分别拖动橙色起振锤和青色拾音梭。
...
- **本地配方铭牌架**：甲、乙、丙三个槽位保存材质、模型、完整共振齿列、落点/起振点、波导弦参数、手势、Note、输入力度、弓速与弓压。铭牌直接显示关键差异，重开编辑器后可一键回炉并试听；旧版槽位缺少的字段会使用兼容默认值。
...
12. 使用“配方铭牌架”把满意的版本压入甲、乙或丙槽。切换材质、共振齿或弓感双轮后点击“回炉试听”，确认对象、DSP、Note、弓速与弓压一起恢复。
...
配方槽写入 Unreal 的本机工程用户设置，不会生成需要提交的团队资产；适合保存个人试听草案。0.36 起槽位使用第二版数据契约，额外保存完整共振齿列和 MIDI/弓奏表演状态。需要团队共享的正式声学资产仍应使用 `UResonanceMaterialProfile`。
...
| 本地配方铭牌往返 | 自动截图路径把硬木波导弦、弓擦、G3、`55%` 弓速、`68%` 弓压与 6 根共振齿压入甲槽，再把弓压改为 `24%` 后回炉；实机铭牌恢复并显示 `68%`，随后以 Note 55 试听 |
